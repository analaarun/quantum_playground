...
def evolve_and_plot_time_dep(
    ...
    def build_observable(obs_name):
        ...
        if obs_name == "electron_nuclei":
            # Return a list of observables for electron and each nucleus
            obs_list = []
            # Add electron observable first
            obs_list.append(embed(sz_12, electron_idx))
            # Add each nuclear observable
            for i in range(electron_idx):  # This ensures we add all nuclei
                obs_list.append(embed(sz_12, i))
            print(f"Created {len(obs_list)} observables for electron_nuclei view")  # Debug print
            return obs_list
    ...
    if obs is not None:
        ...
        if observable_name == "electron_nuclei":
            ...
            # Debug prints
            print(f"Number of observables: {len(obs)}")
            print(f"Number of expectation values: {len(exp_vals_list)}")
            print(f"Shape of first expectation value array: {exp_vals_list[0].shape}")
            
            fig = go.Figure()
            
            # ─────────────────────────────────────────────────────────────────
            # 1) PLOT THE FULL LINES IMMEDIATELY
            #    (so you see everything without pressing "Play")
            # ─────────────────────────────────────────────────────────────────
            # Plot the full lines immediately for all spins
            # Electron trace (red, solid line)
            fig.add_trace(
                go.Scatter(
                    x=times,
                    y=exp_vals_list[0],
            ...
            # Nuclear traces (different colors, dashed lines)
            colors = px.colors.qualitative.Set2
            for i in range(1, len(exp_vals_list)):
                print(f"Adding nuclear trace {i}")  # Debug print
                fig.add_trace(
                    go.Scatter(
                        x=times,
                        y=exp_vals_list[i],
            ...
            for i in range(len(times)):
                ...
                # Add nuclear data
                for j in range(1, len(exp_vals_list)):
                    frame_data.append(
                        go.Scatter(
                            x=times[:i+1],
                            y=exp_vals_list[j][:i+1],
                            mode='lines',
                            name=f'Nucleus {j}',
                            line=dict(
                                dash='dash',
            ...
            fig.update_layout(
                height=600,
                title="Time Evolution of Electron and Nuclear Spins",
                xaxis_title="Time (arbitrary units)",
                yaxis_title="Expectation Value ⟨Sz⟩",
            ...
            # Plot complete data immediately
            fig.add_trace(
                go.Scatter(x=times, y=exp_vals_system, mode='lines', name='System'),
                row=1, col=1
            )
            fig.add_trace(
                go.Scatter(x=times, y=exp_vals_electron, mode='lines', name='Electron'),
                row=2, col=1
            )
            fig.add_trace(
                go.Scatter(x=times, y=exp_vals_nuclear_avg, mode='lines', name='Nuclear'),
                row=3, col=1
            )
            ...
            # Plot complete data immediately
            fig = go.Figure(
                data=[go.Scatter(x=times, y=exp_vals, mode='lines', name=observable_name)]
            )
        ...
        if "energy_levels_context" in st.session_state and st.session_state.energy_levels_context:
            with st.expander("Energy Levels Details", expanded=False):
                st.text_area("", value=st.session_state.energy_levels_context, height=200)
            
        if st.checkbox("Show Plot Details", value=False, key="show_plot_context_checkbox"):
            time_evolution_explanation = f"This shows the time evolution of the observable ⟨{observable_name}⟩ over time.\n\n"
            time_evolution_explanation += f"Key simulation parameters:\n"
            time_evolution_explanation += f"- t_max: {t_max}\n"
            time_evolution_explanation += f"- Number of steps: {num_steps}\n"
            time_evolution_explanation += f"- RF Frequency: {args.get('nu_rf', 'N/A')}\n"
            time_evolution_explanation += f"- RF Amplitude: {args.get('amp_rf', 'N/A')}\n"
            time_evolution_explanation += f"- Initial State Index: {state_index}\n\n"
            time_evolution_explanation += f"Expectation values at {num_steps} time points:\n"
            time_evolution_explanation += "\n".join([
                f"t = {times[i]:.2f}:"
                + f"\nElectron: ⟨Sx⟩ = {expect(electron_ops['x'], result.states[i]):.3f}, "
                + f"⟨Sy⟩ = {expect(electron_ops['y'], result.states[i]):.3f}, "
                                 + f"⟨Sz⟩ = {expect(electron_ops['z'], result.states[i]):.3f}"
                                 + "".join([f"\nNucleus {n+1}: ⟨Sx⟩ = {expect(nuclear_ops[n]['x'], result.states[i]):.3f}, "
                                          + f"⟨Sy⟩ = {expect(nuclear_ops[n]['y'], result.states[i]):.3f}, "
                                          + f"⟨Sz⟩ = {expect(nuclear_ops[n]['z'], result.states[i]):.3f}"
                                          for n in range(num_nuclei)])
                                 + "\n"
                                 for i in np.linspace(0, len(times) - 1, num_steps, dtype=int)
                             ])
            

            with st.expander("Plot and Calculation Details", expanded=True):
                user_prompt = st.text_area(
                    "Context",
                    value=time_evolution_explanation,
                    height=300,
    ...
    if show_bloch_spheres and times_for_bloch:
        ...
        sy_12 = sigmay()/2
        sz_12 = sigmaz()/2
        for t_req in times_for_bloch:
            idx = np.argmin(np.abs(times - t_req))
            actual_t = times[idx]
            psi_t = result.states[idx]
            
            st.markdown(f"### Bloch Spheres at t={actual_t:.3f}")
            with st.container():
                # Create minimum 6 columns or number of spins, whichever is larger
                num_cols = max(6, total_spins)
                cols = st.columns(num_cols)
                
                # Place nuclear and electron Bloch spheres side by side
                for spin_idx in range(total_spins):
                    with cols[spin_idx]:
                        ...
                        # Determine if this is electron or nuclear spin
                        label = "Electron" if spin_idx == electron_idx else f"Nucleus {spin_idx+1}"
                        ...
                        plt.close(fig)
                st.write("---")  # Add separator between different time points
    return result
...
with st.expander("Parameter Settings", expanded=True):
    ...
    with col1:
        ...
        st.session_state.num_nuclei = st.number_input(
        ...
        st.session_state.hbar = st.number_input(
        ...
        st.session_state.Bz = st.number_input(
        ...
        st.session_state.J_dd = st.number_input(
        ...
        st.session_state.nu_rf = st.number_input(
            "RF frequency (MHz):", 
            0.0, 
            50.0, 
        ...
        st.session_state.amp_rf = st.number_input(
    ...
    with col2:
        st.subheader("Nuclear Parameters")
        st.session_state.gamma_n = st.number_input(
            "γ_n gyromagnetic ratio (MHz/T):", 
            0.00, 100.00, st.session_state.get('gamma_n', 1.0), 0.01
        )
        st.session_state.q = st.number_input(
            "q quadrupole coupling (MHz):", 
            -2.0, 2.0, st.session_state.get('q', 0.0), 0.1
        )
        st.session_state.eta = st.number_input(
            "η quadrupole asymmetry (dimensionless):", 
            -1.0, 1.0, st.session_state.get('eta', 0.0), 0.05
        )
        st.session_state.A_hf = st.number_input(
            "A_hf hyperfine coupling (MHz):", 
            0.0, 100.0, st.session_state.get('A_hf', 1.0), 0.1
        )
...
"""
        ...
        # Store context_text in session state
        st.session_state.energy_levels_context = context_text
        if st.button("Copy to Clipboard", key="copy_to_clipboard_button_1"):
            pyperclip.copy(user_prompt)
            st.success("User prompt copied to clipboard!")
        
    # Add separate checkbox for energy levels tab, using session state
    if 'show_bloch_spheres_energy_level' not in st.session_state:
        st.session_state.show_bloch_spheres_energy_level = False
    
    st.session_state.show_bloch_spheres_energy_level = st.checkbox(
        "Show Bloch Spheres", 
        value=st.session_state.show_bloch_spheres_energy_level, 
        key="energy_level_bloch_spheres_tab"
    )
    
    if st.session_state.show_bloch_spheres_energy_level:
        with st.container():
            ...
            for state_idx, energy in enumerate(all_energies):
                ...
                with st.container():
                    # Create minimum 6 columns
                    num_cols = max(6, len(all_nuclear_dms[state_idx]) + 1)  # minimum 6 or number of spins
                    cols = st.columns(num_cols)
                    
                    # Place nuclear Bloch spheres side by side
                    ...
                    # Place electron Bloch sphere next to the last nucleus
                    with cols[len(all_nuclear_dms[state_idx])]:
